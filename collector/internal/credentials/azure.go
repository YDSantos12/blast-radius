...
func collectAzure() []CredentialItem {
	...
	items = append(items, collectAzureAccessTokens(azureDir)...)
	items = append(items, collectAzureMSALCache(azureDir)...)
	return items
}
...
func collectAzureAccessTokens(azureDir string) []CredentialItem {
	...
	for _, e := range entries {
		...
		item := NewCredentialItem("azure_token", path, e.AccessToken)
		item.FoundAt = mtime
...
func collectAzureMSALCache(azureDir string) []CredentialItem {
	...
	for _, tok := range cache.AccessToken {
		...
		item := NewCredentialItem("azure_token", path, tok.Secret)
		item.FoundAt = mtime
		item.Context = map[string]any{
			"tenant_id":       tok.Realm,
			"subscription_id": "",
